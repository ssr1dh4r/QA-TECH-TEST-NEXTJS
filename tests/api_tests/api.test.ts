...
test.describe('API Tests', () => {
    ...
    var testData : ImageTypeDto[];
    
    var api: ImageApiCalls;
    ...
    test('Test to add image using POST', async() => {
        ...
        const getImageData = await api.getAllImages();
        const imageList : ImageTypeDto[] = await getImageData.json();
    ...
    test('Test to delete an image using DELETE', async() => {
        ...
        const deleteData : ImageTypeDto[] = JSON.parse(dataToDelete);
        ...
        if(testData.length > 0){
            ...
            console.log("File has no image record(s) to delete")
        }
    });
